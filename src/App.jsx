...
const WEDDING = {
  ...
  groom: {
    ko: "표민우", ja: "ピョ・ミヌ",
    fatherKo: "표승훈", motherKo: "김영옥",
    fatherJa: "ピョ・スンフン", motherJa: "キム・ヨンオク",
    relationKo: "장남", relationJa: "長男",
    phone: "010-0000-0000", // ← 実際の番号に差し替え
  },
  bride: {
    ko: "오카자키 히나", ja: "岡崎 ひな", // ← 漢字表記に差し替え可
    fatherKo: "오카자키 히로야스", motherKo: "오카자키 리호",
    fatherJa: "岡崎 ヒロヤス", motherJa: "岡崎 リホ",
    relationKo: "장녀", relationJa: "長女",
    phone: "010-0000-0000",
  },
  venue: {
    nameKo: "수원 마이어스 웨딩홀 2F",
    nameJa: "水原マイヤーズウェディングホール 2F",
  ...
  accounts: {
    groomSide: [
      { bankKo: "기업은행", bankJa: "IBK企業銀行", number: "000-000000-00-000", holderKo: "표민우", holderJa: "ピョ・ミヌ" },
    ],
    brideSide: [
      { bankKo: "국민은행", bankJa: "KB国民銀行", number: "000000-00-000000", holderKo: "오카자키 히나", holderJa: "岡崎 ひな" },
    ],
  },
};
...
const GALLERY_IMGS = Array.from({ length: GALLERY_COUNT }, (_, i) =>
...
const T = {
  ko: {
    ...
    dday: (d) => (d > 0 ? `결혼식까지 ${d}일 남았습니다` : d === 0 ? "오늘은 결혼식 날입니다 🤍" : "결혼식이 무사히 끝났습니다"),
    gallery_title: "갤러리",
    ...
    account_note: "참석이 어려우신 분들을 위해 기재했습니다.\n너그러운 마음으로 양해 부탁드립니다.",
    groom_side: "신랑측",
    bride_side: "신부측",
    copy: "복사",
    copied: "복사되었습니다",
  ...
  ja: {
    ...
    sms: "SMS",
    calendar_title: "挙式日時",
    dday: (d) => (d > 0 ? `挙式まで あと${d}日` : d === 0 ? "本日、挙式当日です 🤍" : "挙式は無事に終わりました"),
    gallery_title: "ギャラリー",
    location_title: "アクセス",
...
export default function WeddingInvitation() {
  ...
  const [rsvpForm, setRsvpForm] = useState({ name: "", side: "groom", attend: "yes", meal: "yes", count: 1 });
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(100)
      .then(({ data }) => {
        if (data) setGuestbook(data.map((r) => ({ name: r.name, msg: r.message })));
      });
  }, []);
  ...
  const startDemoPiano = async () => {
    if (!toneRef.current) {
      ...
      const loop = new Tone.Loop((time) => {
        ...
        if (step % 3 === 0) synth.triggerAttackRelease(ch[0].replace(/\d/, (d) => d - 1), "1n", time);
        step++;
      }, "4n");
  ...
  const switchLang = (next) => {
    if (next === lang) return;
    setFade(true);
    setTimeout(() => { setLang(next); setFade(false); }, 220);
  };
  ...
  const copyText = (text) => {
    const done = () => { setToast(t.copied); setTimeout(() => setToast(""), 1600); };
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ ...S.card, opacity: fade ? 0 : 1, transition: "opacity .22s ease" }}>
        ...
        <section style={{ ...S.section, background: "#F6F1EA" }}>
          ...
          {[
            {
              f: lang === "ko" ? WEDDING.groom.fatherKo : WEDDING.groom.fatherJa,
              m: lang === "ko" ? WEDDING.groom.motherKo : WEDDING.groom.motherJa,
              r: lang === "ko" ? WEDDING.groom.relationKo : WEDDING.groom.relationJa,
              n: names.g, phone: WEDDING.groom.phone,
            },
            {
              f: lang === "ko" ? WEDDING.bride.fatherKo : WEDDING.bride.fatherJa,
              m: lang === "ko" ? WEDDING.bride.motherKo : WEDDING.bride.motherJa,
              r: lang === "ko" ? WEDDING.bride.relationKo : WEDDING.bride.relationJa,
              n: names.b, phone: WEDDING.bride.phone,
            },
          ].map((fam, i) => (
        ...
        <section style={S.section}>
          ...
          <div style={S.calWrap}>
            ...
            <div style={S.calGrid}>
              {(lang === "ko" ? ["일","월","화","수","목","금","토"] : ["日","月","火","水","木","金","土"]).map((d, i) => (
                <div key={d} style={{ ...S.calHead, color: i === 0 ? "#C77" : C.sub }}>{d}</div>
              ))}
              {calCells.map((d) => (
                <div key={d} style={{ ...S.calCell, ...(d === 1 ? S.calToday : {}) }}>{d}</div>
              ))}
            </div>
          </div>
        ...
        <section style={{ ...S.section, background: "#F6F1EA" }}>
          ...
          <div style={S.galleryGrid}>
            {GALLERY_IMGS.map((src, i) => (
              <div key={i} style={S.galleryCell} onClick={() => setLightbox(src)}>
        ...
        <section style={S.section}>
          ...
          <p style={{ ...S.body, marginBottom: 4, fontWeight: 600 }}>{venueName}</p>
          <p style={{ ...S.sub, whiteSpace: "pre-line" }}>{venueAddr}{"\n"}Tel. {WEDDING.venue.tel}</p>
          ...
          {lang === "ja" ? (
            <>
              ...
              <a href={googleUrl} target="_blank" rel="noreferrer" style={{ ...S.wideBtn, marginTop: 10 }}>
                {t.map_open_google}
              </a>
            </>
          ) : (
            <>
              <div style={S.mapPlaceholderKo}>
                ...
                <span style={{ fontSize: 13, color: C.sub, marginTop: 6 }}>수원 마이어스 웨딩홀</span>
              </div>
              <p style={S.sub}>{t.map_note}</p>
              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                <a href={kakaoUrl} target="_blank" rel="noreferrer" style={{ ...S.mapBtn, background: "#FEE500", color: "#3B1E1E", border: "none" }}>{t.map_open_kakao}</a>
                <a href={naverUrl} target="_blank" rel="noreferrer" style={{ ...S.mapBtn, background: "#03C75A", color: "#fff", border: "none" }}>{t.map_open_naver}</a>
                <a href={tmapUrl} target="_blank" rel="noreferrer" style={S.mapBtn}>{t.map_open_tmap}</a>
              </div>
            </>
          )}
        ...
        <section style={{ ...S.section, background: "#F6F1EA" }}>
          ...
          <p style={{ ...S.sub, whiteSpace: "pre-line", marginBottom: 22 }}>{t.account_note}</p>
          {[
            ...
            <div key={grp.label} style={{ marginBottom: 16 }}>
              <p style={{ ...S.sub, fontSize: 12, letterSpacing: 2, marginBottom: 8 }}>{grp.label}</p>
              {grp.list.map((a, i) => (
                <div key={i} style={S.accountCard}>
                  <div style={{ textAlign: "left" }}>
                    <p style={{ margin: 0, fontSize: 13, color: C.sub }}>
                      {lang === "ko" ? a.bankKo : a.bankJa} · {lang === "ko" ? a.holderKo : a.holderJa}
                    </p>
                    <p style={{ margin: "4px 0 0", fontSize: 15, letterSpacing: 0.5 }}>{a.number}</p>
                  </div>
                  <button onClick={() => copyText(a.number)} style={S.miniBtn}>{t.copy}</button>
                </div>
              ))}
            </div>
          ))}
        ...
        <section style={S.section}>
          ...
          {rsvpDone ? (
            ...
            <div style={{ textAlign: "left" }}>
              ...
              <div style={S.segRow}>
                {[["groom", t.groom_side], ["bride", t.bride_side]].map(([v, lb]) => (
                  <button key={v} onClick={() => setRsvpForm({ ...rsvpForm, side: v })}
                    style={{ ...S.seg, ...(rsvpForm.side === v ? S.segOn : {}) }}>{lb}</button>
                ))}
              </div>
              <label style={S.formLabel}>{t.rsvp_attend}</label>
              <div style={S.segRow}>
                {[["yes", t.attend_yes], ["no", t.attend_no]].map(([v, lb]) => (
                  <button key={v} onClick={() => setRsvpForm({ ...rsvpForm, attend: v })}
                    style={{ ...S.seg, ...(rsvpForm.attend === v ? S.segOn : {}) }}>{lb}</button>
                ))}
              </div>
              {rsvpForm.attend === "yes" && (
                <>
                  ...
                  <div style={S.segRow}>
                    {[["yes", t.meal_yes], ["no", t.meal_no]].map(([v, lb]) => (
                      <button key={v} onClick={() => setRsvpForm({ ...rsvpForm, meal: v })}
                        style={{ ...S.seg, ...(rsvpForm.meal === v ? S.segOn : {}) }}>{lb}</button>
                    ))}
                  </div>
                  ...
                  <input
                    ...
                    onChange={(e) => setRsvpForm({ ...rsvpForm, count: Number(e.target.value) })}
                    style={S.input}
                  />
                </>
              )}
              <button onClick={submitRsvp} style={{ ...S.wideBtn, marginTop: 8 }}>{t.rsvp_submit}</button>
            </div>
          )}
        </section>
        ...
        <section style={{ ...S.section, background: "#F6F1EA" }}>
          ...
          <div style={{ textAlign: "left" }}>
            ...
            />
            <button onClick={submitGb} style={S.wideBtn}>{t.gb_submit}</button>
          </div>
          <div style={{ marginTop: 22 }}>
            {guestbook.length === 0 ? (
              ...
              guestbook.map((g, i) => (
                <div key={i} style={{ ...S.accountCard, alignItems: "flex-start", flexDirection: "column", gap: 4 }}>
                  <p style={{ margin: 0, fontSize: 13, color: C.accent, fontWeight: 600 }}>{g.name}</p>
                  <p style={{ margin: 0, fontSize: 14, whiteSpace: "pre-line", textAlign: "left" }}>{g.msg}</p>
                </div>
              ))
            )}
          </div>
        ...
        <section style={{ ...S.section, paddingBottom: 64 }}>
          <div style={S.hairline} />
          <p style={{ ...S.sub, fontFamily: "'Cormorant Garamond', serif", fontSize: 15, letterSpacing: 3 }}>
            {names.g} & {names.b}
          </p>
      ...
      >
        {"♪"}
        <span style={{ fontSize: 9, letterSpacing: 1, display: "block" }}>{bgmOn ? "ON" : "OFF"}</span>
      </button>
...
function styles(bodyFont) {
  return {
    ...
    sub: { fontSize: 13, lineHeight: 1.9, color: C.sub, margin: 0, whiteSpace: "pre-line" },
    hairline: { width: 36, height: 1, background: C.accent, margin: "26px auto" },
    heroPhoto: {
      width: "100%",
      aspectRatio: "3/4",
      background: "#EFE7DB",
    ...
    heroNames: { fontSize: 24, fontWeight: 600, margin: 0, letterSpacing: 1, lineHeight: 1.6 },
    heroAnd: {
      display: "block",
      fontFamily: "'Cormorant Garamond', serif",
      fontStyle: "italic",
    ...
    calWrap: { background: C.card, border: `1px solid ${C.line}`, borderRadius: 10, padding: "18px 14px" },
    calMonth: { fontFamily: "'Cormorant Garamond', serif", letterSpacing: 4, fontSize: 13, color: C.sub, margin: "0 0 12px" },
    calGrid: { display: "grid", gridTemplateColumns: "repeat(7, 1fr)", rowGap: 8 },
    calHead: { fontSize: 11 },
    ...
    lightboxImg: { maxWidth: "100%", maxHeight: "92vh", borderRadius: 4, objectFit: "contain" },
    bgmBtn: {
      position: "fixed",
    ...
    transportLabel: { fontSize: 12, letterSpacing: 2, color: C.accent, margin: "18px 0 6px", fontWeight: 600 },
    transportBody: { fontSize: 13, lineHeight: 1.9, color: C.ink, whiteSpace: "pre-line", margin: 0 },
    accountCard: {
      background: C.card,
      border: `1px solid ${C.line}`,
      borderRadius: 8,
    ...
    formLabel: { display: "block", fontSize: 12, color: C.sub, letterSpacing: 1, margin: "14px 0 6px" },
    input: {
      width: "100%",
      border: `1px solid ${C.line}`,
      borderRadius: 8,
